//
// This file is included in another code DisplayEvent_circleFitter.C
//    in order to use circle-fitting functions from that code
//
...
void initialize(int debug, int arrSize, XYPoint *points);//(XYPoint point [])
...
XYPoint circumcenter(int debug, XYPoint pI, XYPoint pJ, XYPoint pK, bool &Collinearity); //This may have memory loss issues (not sure for now)
int minimize(int debug, int iterMax, double innerThreshold, double outerThreshold, int arrSize, XYPoint *points);
...
void circleFitter4DisplayEvent(int debug) { 

  const int nPoints=21;  XYPoint p2DAr[nPoints]; //20 from RTPC hit positions and 1 for the origin (0,0)
   
  //This g4 track could be fitted even before I properly managed aligned-triplets that crashed fit on above track.
  double xArr[nPoints] = {0, 0.566571, 1.03486, 1.30482, 1.35623, 1.48295, 1.60786, 1.63137, 1.74832, 1.86313, 1.96621, 2.07043, 
			  1.84176, 1.94262, 2.04109, 1.90568, 1.90197, 1.98822, 2.07203, -2.01642, -2.0994};
  double yArr[nPoints] = {0, 3.84776, 3.89603, 3.81411, 3.64512, 3.59543, 3.54133,  3.37065, 3.3115, 3.24829, 3.01258, 2.94193, 
			  2.91433, 2.8481, 2.77838, 2.68792, 2.49797,  2.42989, 2.35883, 2.40654, 2.3345}; 
   
  for(int i=0;i<nPoints;i++) { p2DAr[i].SetX(xArr[i]);  p2DAr[i].SetY(yArr[i]);};
  //cout<<"p2DAr.Size() = "<<p2DAr.GetSize()<<endl;

  initialize(debug, nPoints, p2DAr);
  ...
  //for(int i=0;i<5;i++)
    ...
    int iter = minimize(debug, 100, 0.1, 1.0e-12, nPoints, p2DAr);
...
void circleFitter(int debug) {
  ...
  if(debug==1) cout<<p1.x()<<"  "<<p3.y()<<"  "<<p4.z()<<"  "<<p5.z()<<endl;
  ...
  if(debug==1) cout<<p1n.x()<<"  "<<p2n.z()<<"  "<<p3n.y()<<"  "<<p4n.z()<<"  "<<p5n.z()<<endl;
  ...
  if(debug==1) {for(int i=0;i<5;i++) cout<<pAr[i].x()<<" "<<pAr[i].y()<<" "<<pAr[i].z()<<" \n";}
...
void initialize(int debug, int arrSize, XYPoint *points)//(XYPoint point [])
{
  ...
  XYPoint origin; origin.SetX(0); origin.SetY(0); //7/19/16
  int nTriplets = 0; //Number of non-aligned triplets
  xCenter=0.0, yCenter=0.0;

  /*
  //kp: This block uses all possible triplets to make the initial estimate of the circle-fit
  //for(int i=0;i<5;i++) cout<<2*point[i].x()<<" "<<3*point[i].y()<<" "<<4*point[i].z()<<" \n";
  ...
  */
  ...
  /*
  //kp: In this block I want to try only those triplets which always uses the origin as one of the points.//7/19/16
  //for(int i=0;i<5;i++) cout<<2*point[i].x()<<" "<<3*point[i].y()<<" "<<4*point[i].z()<<" \n";
  ...
  */
  ...
  //kp: In this block I want to try only those triplets which always uses the origin as one of the points.
  //    additionally, I want to pair hits above half-way point to the initial hit
  ...
  for (int i = 2; i < arrSize-1; i++) {
    ...
    if(i< arrSize/2) { p3 = points[arrSize-1];  p2 = points[i]; }
    else             { p3 = points[1]; p2 = points[i]; }

    
    circumCenter = circumcenter(debug, p1,p2,p3,Collinearity); //6/16/16
    if(Collinearity==true) continue; //artificially given large value for circumcenter of aligned triplets

    kpX = circumCenter.x(); kpY = circumCenter.y(); kpP1X = p1.x(); kpP1Y = p1.y();
    if(debug==1) cout<<"iniCenter (x,y): "<<kpX<<","<<kpY<<endl;

    if(!(kpX==kpX) || !(kpY==kpY) || !(kpP1X==kpP1X) || !(kpP1Y==kpP1Y) ) continue; //Avoid cases with 'nan' values
    
    kpRadius = sqrt( pow(kpX-kpP1X,2) + pow(kpY-kpP1Y,2)); if(!(kpRadius==kpRadius)) continue;

     //kp: Non-essential part
    h1xIniCent->Fill(kpX); h1yIniCent->Fill(kpY); h1rIniCent->Fill(kpRadius); 
    ...
    nTriplets++;
    xCenter += circumCenter.x(); //kp: Making the sums of xCenter (to get average below)
    yCenter += circumCenter.y(); //kp: Making the sums of yCenter (to get average below)
  }



  if(debug==1) cout<<"kp: nTriplets = "<<nTriplets<<endl;
  if(debug==1) cout<<"Initial estimate of the circle fit: Sums of xCenter & yCenter are: "<<xCenter<<"&"<<yCenter<<endl;  
 if(nTriplets>0)
    {
      xCenter /= nTriplets;
      yCenter /= nTriplets;
    }
  if(debug==1) cout<<"Initial estimate of the circle fit: Center at ("<<xCenter<<","<<yCenter<<") & ";  
  updateRadius(arrSize, points);
  if(debug==1) cout<<" Radius="<<rHat<<endl; 
  double xCent[1]={xCenter}, yCent[1]={yCenter}; 
  grIniCent = new TGraph(1,xCent,yCent);
...
XYPoint circumcenter(int debug, XYPoint pI, XYPoint pJ, XYPoint pK, bool &Collinearity) //This may have memory loss issues (not sure for now)
{
  ...
  if(debug==1) cout<<"Inside circumcenter(..):\n";
  if(debug==1) cout<<dJK.x()<<" "<<dIJ.y()<<" "<<dIJ.x()<<"  "<<dJK.y()<<";  det = "<<det<<endl;
 ...
 */
void computeCost(int debug, int arrSize, XYPoint *points);
double newtonStep(int debug, double u, double v, int arrSize, XYPoint *points);
int minimize(int debug, int iterMax, double innerThreshold, double outerThreshold,
	     int arrSize, XYPoint *points)
{
  if(debug==1) cout<<"Now in minimze(..) function !!!"<<endl;
  computeCost(debug, arrSize, points);
  if ((J < 1.0e-10) || (sqrt(dJdx * dJdx + dJdy * dJdy) < 1.0e-10)) 
    {
      // we consider we are already at a local minimum
      if(debug==1) cout<<"kp: We're already at a local minimum. So no further minimization ...\n";
      return 0;
    }
  ...
  for (int iterations = 0; iterations < iterMax;) 
	...
	{
	  innerJ = J;
	  double lambda = newtonStep(debug, u, v, arrSize, points);
...
//void computeCost() {
void computeCost(int debug, int arrSize, XYPoint *points) {
  if(debug==1) cout<<"Inside computeCost(..) !!!\n";
    ...
    for (int i = 0; i < arrSize; ++i) {
      ...
      */
      ...
      dJdx += dx * ratio;
      dJdy += dy * ratio;
      if(debug==1) cout<<"dx, dy, di  J, dJdx, dJdy xCent yCent: "<<dx<<"  "<<dy<<"  "<<di
		       <<"  "<<J<<"  "<<dJdx<<"  "<<dJdy<<"  "<<xCenter<<"  "<<yCenter<<endl;
    }
    dJdx *= 2.0;
    dJdy *= 2.0;
}
...
void drawPlots4DisplayEvent(int nPoints, double *xPoints, double *yPoints, double xCentre, double yCentre, double Radius)
{
  cout<<"Inside drawPlots() ... Center is ("<<xCenter<<","<<yCenter<<") & R="<<Radius<<endl;
  ...
  char fNm[400]; 
  /*
  //TCanvas *c3 = new TCanvas("c3","c3",600,600); //c3->Divide(2,2,0.0001,0.0001);
  TVirtualPad *pad = (TVirtualPad*)c3->cd(3); pad->SetGrid();  //c3->SetGrid();
  //c3->DrawFrame(-30.0,-30.0,120.0,120.0);//c3->DrawFrame(-5,-5,5,5);
  pad->DrawFrame(gMin-0.1*gD,gMin-0.1*gD,gMax+0.1*gD,gMax+0.1*gD,"Y vs X");//Using global min/max for real looking circles
  */
  // TVirtualPad *pad = (TVirtualPad*)c3->cd(1); pad->SetGrid();  //c3->SetGrid();x
  //gr->Draw("P"); grIniCent->Draw("P"); grFinCent->Draw("P"); arc->Draw("same");
  arc->Draw("same");
 
  TVirtualPad *pad = (TVirtualPad*)c3->cd(1); 
  grIniCent->Draw("P"); grFinCent->Draw("P"); arc->Draw("same");
  //arc->Draw("same"); gr->Draw("P"); 
  //TEllipse el1(0.,0.,100.,100.); el1.Draw();
  myTLatex(0.035,6,0.12,0.92,"Initial Estimate of Center");
  myTLatex(0.035,2,0.12,0.88,"Final Estimate of Center");
